...
import { listUserSubmissions } from '../lib/api';
...
const buildMoreLink = (username: string, page: number) => {
  ...
  return `/submitted?${params.toString()}`;
};

export const SubmittedPage = ({ config }: { config: RuntimeConfig }) => {
  ...
  useEffect(() => {
    ...
    listUserSubmissions(config, username, { page })
      .then((response) => {
        if (!cancelled) {
  ...
  return (
    <Layout currentPage="submitted">
      <div className="activity-page-intro">Stories by {username || 'user'}</div>
      {error ? (
        ...
        <div className="activity-page-empty">No submissions.</div>
      )}
    </Layout>
  );
};
