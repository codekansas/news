...
const main = async () => {
  ...
  const [homeResponse, configResponse, healthResponse, feedResponse] = await Promise.all([
    fetch(siteUrl),
  ...
  assert(homeHtml.includes('Hacker News'), 'Home page did not render the Hacker News shell.');
